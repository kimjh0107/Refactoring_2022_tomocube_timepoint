...
class CNN(nn.Module):
    def __init__(self, in_channels = 1, num_classes = 2):
        ...
        self.conv1 = nn.Conv3d(in_channels=1, out_channels=32, kernel_size=3, stride = 1)
        self.conv2 = nn.Conv3d(in_channels=32, out_channels=64, kernel_size=3, stride = 1)
        self.conv3 = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, stride = 1)
        self.conv4 = nn.Conv3d(in_channels=64, out_channels=128, kernel_size=3, stride = 1)
        self.conv5 = nn.Conv3d(in_channels=128, out_channels=256, kernel_size=3, stride = 1)
        self.conv6 = nn.Conv3d(in_channels=256, out_channels=512, kernel_size=3, stride = 1)

       # self.conv5 = nn.Conv3d(in_channels=128, out_channels=128, kernel_size=3, stride = 1, padding=1)
        self.pool = nn.MaxPool3d(kernel_size=(2,2,2))
        self.dropout = nn.Dropout(0.2)
        self.fc = nn.Linear(150528, num_classes)
...
#     def __init__(self, in_channels = 1, num_classes = 2):
#         super(CNN, self).__init__()
#         self.conv1 = nn.Conv3d(in_channels=1, out_channels=12, kernel_size=3, stride = 2)
#         self.conv2 = nn.Conv3d(in_channels=12, out_channels=24, kernel_size=3, stride = 2)
#         self.conv3 = nn.Conv3d(in_channels=24, out_channels=64, kernel_size=3, stride = 2)
#         self.conv4 = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, stride = 2)
...
class ResNet(nn.Module):  # [3,4,6,3]-> list , tell us how many times we want to reuse or want to use this block right here 
    def __init__(self, block, layers, image_channels, num_classes):
        ...
        self.layer4 = self._make_layer(block, layers[3], intermediate_channels=128, stride=2)
